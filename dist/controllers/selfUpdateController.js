...
class SelfUpdateController {
    constructor() {
        ...
        this.executeStart = (serviceName) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            yield get_installed_path_1.getInstalledPath('sw-db-sync').then((path) => {
                config.npmPath = path;
            });
            if (config.currentVersion < config.latestVersion) {
                yield console_1.consoleCommand(`cd ${config.npmPath}; rm -rf dist`, false);
                yield download_git_repo_1.default('jellesiderius/sw-db-sync#master', config.npmPath, function (err) {
                    return tslib_1.__awaiter(this, void 0, void 0, function* () {
                        yield console_1.consoleCommand(`cd ${config.npmPath}; npm install`, false);
                        console_1.success(`Updated sw-db-sync from ${config.currentVersion} to ${config.latestVersion}`);
                    });
                });
            }
            else {
                console_1.success(`sw-db-sync is already up to date`);
            }
            return true;
        });
    }
